        ...
        <div className="p-8 sm:p-10">
          <div className="text-center mb-8">
            <h2 className="font-display text-3xl font-extrabold text-primary tracking-tight">
              {view === "login" ? t("welcomeBack") : t("joinLifeWise")}
            </h2>
            <p className="text-muted font-body mt-2">
              {view === "login" ? t("signInSubtitle") : t("registerSubtitle")}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? <Loader2 className="animate-spin" size={20} /> : view === "login" ? tNav("signIn") : t("createAccount")}
            </button>
          </form>
          ...
          <div className="mt-8">
            <div className="relative flex items-center justify-center mb-6">
              ...
              <span className="relative bg-surface/90 px-4 text-xs font-bold text-muted uppercase tracking-widest">{t("orContinueWith")}</span>
            </div>
            ...
            >
              <FcGoogle size={24} /> {t("google")}
            </button>
          </div>

          <p className="text-center mt-10 text-muted font-ui text-sm">
            {view === "login" ? t("newToLifeWise") : t("alreadyHaveAccount")}{" "}
            <button 
              onClick={() => setView(view === "login" ? "register" : "login")}
              className="text-accent font-bold hover:underline"
            >
              {view === "login" ? t("joinNow") : t("signInHere")}
            </button>
          </p>
        </div>
      </div>
